...
export const useCanvasChart = ({
  ...
  // }, [width, height, margin]);
   ...
   // CORRECTION : Création des échelles D3 avec d3.scaleTime
  const createScales = useCallback((data) => {
    if (!data.length) return {};
    ...
    // Échelle temporelle pour l'axe X
    const xScale = d3.scaleTime() // <- CHANGEMENT CLÉ
      .domain(d3.extent(data, d => d.time)) // Utilise le min et max du temps
      .range([0, innerWidth]);
